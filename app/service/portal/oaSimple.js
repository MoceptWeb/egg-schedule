...
class OaService extends Service {
    ...
    */   
    ...
    async updateUser(oaUser) {
        ...
        const updateUser = await Promise.all(promiseAll);
    ...
    async updateUserByName(oaUser) {
        const res = await this.ctx.helper.mysqlQuery('UPDATE user set username = ?  where id = ?', [oaUser + 'USERNA11ME', oaUser]);
        let update = null;
        // if(res && res.length === 1) {
